...
$(document).ready(function () {
  ...
  $('#version_booking .version-payment-section').removeAttr('disabled');
  ...
  $('#show_booking .show-payment-section').removeAttr('disabled');
  ...
  $(document).on("keyup change", '.change', function (event) {
    ...
    console.log(changeFeild);
    console.log(status);
    console.log(cal_selling_price);
    console.log(key);

    if (status && status == 'booking' && cal_selling_price == false) {
      getBookingDetailValues(key, changeFeild);
    } else {
  ...
  $(document).on('click', '.bookings-service-category-btn', function (e) {
    ...
    if (category_id) {
      setTimeout(function () {
        ...
        if (markupType == 'whole') {
          $("".concat(quoteClass)).find('.actual-cost').attr("data-status", "booking");
          $("".concat(quoteClass)).find('.actual-cost').data("status", "booking");
          $("".concat(quoteClass)).find('.markup-amount').attr("readonly", true);
          $("".concat(quoteClass)).find('.markup-percentage').attr("readonly", true);
          $("".concat(quoteClass)).find('.cal_selling_price').removeAttr('checked');
        }
  ...
  $(document).on('click', '.bookings-service-category-btn-below', function (e) {
    ...
    if (category_id) {
      setTimeout(function () {
        ...
        quote.addClass("quote-".concat(quoteKey));
        quote.find('.prod-feild-col').remove();
        $("".concat(quoteClass)).find('.finance .row:not(:first):not(:last)').remove(); // $(`${quoteClass}`).find('.actual-cost').attr("data-status", "");
  ...
  */
  ...
  function getBookingDetailValues(key, changeFeild) {
    ...
    if (changeFeild == 'actual_cost') {
      ...
      if (markupType == 'whole') {
        var totalNetPrice = removeComma($('.total-net-price').val());
        var totalMarkupPercentage = removeComma($('.total-markup-percent').val());
        var totalMarkupAmount = parseFloat(totalNetPrice / 100) * parseFloat(totalMarkupPercentage);
  ...
  */
  ...
  window.getBookingTotalValues = function () {
    ...
    if (markupType == 'whole') {
      ...
      var totalMarkupAmount = removeComma($(".total-markup-amount").val());

      var _sellingPriceInBookingCurrency = parseFloat(actualCostInBookingCurrency) + parseFloat(totalMarkupAmount);

      $(".total-selling-price").val(check(_sellingPriceInBookingCurrency));
    }
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        if (response && response.sale_person_currency != null) {
          ...
          $('.sale-person-currency-id').attr('data-currency_code', response.sale_person_currency.code).data('currency_code', response.sale_person_currency.code);
        }
  ...
  function findReferenceData(reference_no) {
    $.ajax({
      ...
      success: function success(data) {
        ...
        var leadPassenger = data.response.lead_passenger;
        ...
        if (response) {
          ...
          if (brand.hasOwnProperty('name') && brand.name !== null) {
            setTimeout(function () {
              $("#holiday_type_id option:contains(" + brand.name + ")").attr('selected', 'selected').change(); // $("#holiday_type_id option[data-value='" + data.response.brand.name +"']").attr("selected","selected");
            }, 500);
          }

          if (response.sale_person !== null) {
            // $('#sale_person_id').val(data.response.sale_person).trigger('change');
            $("#sale_person_id option[data-email=\"".concat(response.sale_person, "\"]")).prop('selected', 'selected').change();
          }

          if (response.currency !== null) {
            $("#currency_id option[data-code=\"".concat(response.currency, "\"]")).prop('selected', 'selected').change();
          }
