  ...
  */
  ...
  window.getQuoteTotalValues = function () {
    ...
    if (markupType == 'whole') {
      ...
      var _sellingPriceInBookingCurrency = parseFloat(estimatedCostInBookingCurrency) + parseFloat(totalMarkupAmount);
  ...
  window.getQuoteDetailsValues = function (key, changeFeild) {
    ...
    var rateType = $("input[name=rate_type]:checked").val();
    var markupType = $("input[name=markup_type]:checked").val();
    var estimatedCost = removeComma($("#quote_".concat(key, "_estimated_cost")).val());
  ...
  $(document).on('click', '.quotes-service-category-btn-below', function (e) {
    ...
    if (category_id) {
      setTimeout(function () {
        ...
        quote.find('.prod-feild-col').remove(); // $(`#quote_${quoteKey}_date_of_service`).val(todayDate());
