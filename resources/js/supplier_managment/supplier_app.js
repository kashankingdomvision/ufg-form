$(document).ready(function() {
    ...
    */
    ...
    $(document).on('click', '.supplier-bulk-action-item', function() {
        ...
        if(['delete'].includes(bulkActionType)){
            ...
            if(checkedValues.length > 0){
                ...
                switch(bulkActionType) {
                    case "delete":
                        message    = 'You want to Delete Suppliers?';
                        buttonText = 'Delete';
                        break;
                }
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'POST',
                            url: $('#supplier_bulk_action').attr('action'),
                            data: new FormData($('#supplier_bulk_action')[0]),
                            contentType: false,
                            cache: false,
                            processData: false,
                            success: function(response) {
    ...
    $(document).on('click', '.delete-supplier', function(event) {
        ...
        event.preventDefault();
        let url = $(this).attr('action');
        message    = 'You want to Delete Supplier?';
        buttonText = 'Delete';
