$(document).ready(function() {
    ...
    */
    ...
    $(document).on('click', '.group-owner-bulk-action-item', function() {
        ...
        if(['delete'].includes(bulkActionType)){
            ...
            if(checkedValues.length > 0){
                ...
                switch(bulkActionType) {
                    case "delete":
                        message    = 'You want to Delete Group Owners?';
                        buttonText = 'Delete';
                        break;
                }
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'POST',
                            url: $('#group_owner_bulk_action').attr('action'),
                            data: new FormData($('#group_owner_bulk_action')[0]),
                            contentType: false,
                            cache: false,
                            processData: false,
                            success: function(response) {
    ...
    $(document).on('click', '.delete-group-owner', function(event) {

        event.preventDefault();
        let url = $(this).attr('action');
        message    = 'You want to Delete Group Owner?';
        buttonText = 'Delete';
