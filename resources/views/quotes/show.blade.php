          ...
          <div class="col-md-12">
            <div class="card card-secondary">
              <div class="card-header">
                ...
                <a href="{{ url()->previous() }}" class="btn btn-dark btn-sm float-right" title="Back">
                ...
                @if($quote->booking_status == 'quote')
                  ...
                  <a href="{{ route('quotes.edit', encrypt($quote->id)) }}" onclick="return confirm('Are you sure you want to Edit this Quotation?');" data-title="Edit" data-target="#edit" title="Edit">
                    <button type="button" class="mr-2 btn btn-primary btn-sm float-right" data-title="" data-target="#" title="Edit">Edit Quote</button>
                  </a>
                @endif
                ...
                @if($quote->booking_status == 'cancelled')
                  <a href="{{ route('quotes.restore', encrypt($quote->id)) }}" onclick="return confirm('Are you sure you want to Restore this Quote?');" data-title="Cancel" title="Restore Quote">
                    <button type="button" class="mr-2 btn btn-success btn-sm float-right">Restore Quote</button>
                  </a>
                @endif
              </div>
                                ...
                                <div class="form-group">
                                  ...
                                  <select name="quote[{{ $key }}][category_id]" data-name="category_id" id="quote_{{ $key }}_category_id" class="form-control select2single category-select2 category-id @error('category_id') is-invalid @enderror">
                                    ...
                                    @foreach ($categories as $category)
                                      <option value="{{ $category->id }}" data-slug="{{ $category->slug }}" data-enddateofservice="{{ $category->set_end_date_of_service }}" data-name="{{ $category->name }}" {{ ($q_detail->category_id == $category->id)? 'selected' : NULL}} > {{ $category->name }} </option>
                                    @endforeach
                                  </select>
                              ...
                              <div class="col-sm-2">
                                <div class="form-group">
                                  ...
                                  <div class="input-group">
                                    ...
                                    <input type="number" step="any" value="{{ \Helper::number_format($q_detail->estimated_cost) }}" name="quote[{{ $key }}][estimated_cost]" data-name="estimated_cost" id="quote_{{ $key }}_estimated_cost" class="form-control estimated-cost change-calculation">
                                  </div>
                                </div>
                              </div>
                              ...
                              <div class="col-sm-2 whole-markup-feilds {{ $quote->markup_type == 'whole' ? 'd-none' : '' }}">
                                <div class="form-group">
                                  ...
                                  <div class="input-group">
                                    ...
                                    <input type="number" step="any" value="{{ \Helper::number_format($q_detail->markup_amount) }}" name="quote[{{ $key }}][markup_amount]" data-name="markup_amount" id="quote_{{ $key }}_markup_amount" class="form-control markup-amount change-calculation" >
                                  </div>
                                </div>
                              </div>
                    ...
                    <div class="col-sm-2">
                      <div class="form-group">
                        <div class="input-group">
                          ...
                          <input type="number" value="{{ \Helper::number_format($quote->markup_amount) }}"  step="any" class="form-control total-markup-amount total-markup-change hide-arrows" step="any" min="0" name="total_markup_amount" data-name="total_markup_amount"   value="0.00" readonly>
                        </div>
                      </div>
                    </div>
                    <div class="col-sm-2">
                      <div class="form-group">
                        <div class="input-group">
                          ...
                          <input type="number" value="{{ \Helper::number_format($quote->markup_percentage) }}"  step="any" class="form-control total-markup-percent total-markup-change hide-arrows" min="0" name="total_markup_percent" data-name="total_markup_percent" value="0.00" readonly>
                          <div class="input-group-append">
                            <div class="input-group-text">%</div>
                          </div>
                        </div>
