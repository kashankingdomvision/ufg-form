            ...
            <div class="form-group">
                ...
                <select class="form-control select2-multiple view-rate-booking-currency-filter"  data-placeholder="Select Booking Currency" multiple name="booking_currency[]">
                  @foreach ($currencies as $curren)
                    <option value="{{ $curren->code }}" data-image="data:image/png;base64, {{$curren->flag}}" > &nbsp; {{$curren->code}} - {{$curren->name}} </option>
                  @endforeach
                </select>
            </div>
          </div>
                ...
                <div class="table-responsive table-wrap">
                  <table class="table table-hover text-nowrap">
                    ...
                    <tbody id="currency_conversions">
                        @forelse ($currency_conversions as $key => $value)
...
{{-- {{ (old('booking_currency') == $curren->code)? 'selected': ( (!empty(request()->get('booking_currency')))? (((in_array($curren->code, request()->get('booking_currency'))))? 'selected' : null) : '') }} --}}
